...
# Cache for question_number -> answer_index
question_cache = {}

def extract_index(text):
    print(f"Finding index in {text}")
...
async def handle_question_dumb(packet: QuestionStartPacket, client: KahootClient):
    ...
    # If we've already cached this question, send answer immediately
    if question_number in question_cache:
        answer_index = question_cache[question_number]
        await client.send_packet(RespondPacket(client.game_pin, answer_index, question_number))
        print(f"📤 Sending cached answer for question {question_number}")
        return

    # Otherwise, compute and cache the answer
    ...
    try:
        print(f"🧠 Getting answer for question {question_number}...")
        ...
        print(f"✅ Cached answer for question {question_number}: {answer_index}")
        question_cache[question_number] = answer_index
    except Exception as e:
        print(f"❌ Error getting LLM response: {e}")
        question_cache[question_number] = 0  # fallback
...
async def amain():
    ...
    await client.join_game(966294, "Galaxy")
